...
import MemoryBackedByFileSystemCacheItem from './memory_backed_by_file_system_cache_item';

import logger from '@mfdlabs/logging';

/*
	File Name: memory_backed_by_file_system_cache_repository.ts
	Written by: Nikita Petko
*/

/**
 * A cache repository that stores cache items in memory
 */
export default class MemoryBackedByFileSystemCacheRepository extends BaseCacheRepository {
  /**
   * Constructs a new MemoryBackedByFileSystemCacheRepository instance
   ...
   */
  protected _constructCacheItem(key: string, ttl?: number): MemoryBackedByFileSystemCacheItem {
    return new MemoryBackedByFileSystemCacheItem(key, ttl);
  }
}
